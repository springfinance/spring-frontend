...
const WithdrawSection = ({ pool, index, sharesBalance }) => {
  ...
  const handleSliderChangeCommitted = (_, sliderInt) => {
    ...
    if (sliderInt > 0 && sliderInt < 99) {
      amount = underliyngBalance.times(sliderInt).div(100).decimalPlaces(pool.tokenDecimals, BigNumber.ROUND_DOWN);
  ...
  const handleInputAmountChange = event => {
    ...
    const sliderInt = underliyngBalance.isZero() ? 0 : amount.times(100).dividedToIntegerBy(underliyngBalance).toNumber();
        ...
        {pool.status === 'refund' ? (
          ...
          <div>
            {withdrawSettings.isNeedApproval ? (
              ...
              <div className={classes.showDetailButtonCon1}>
              <h4 className={classes.h5}>Rewards <span>12.12 SPRING</span></h4>
              {!withdrawSettings.isSwap && (
                  <Button
                    className={`${classes.showDetailButton1} ${classes.showDetailButtonContained}1`}
                    type="button"
                    color="primary"
                    disabled={sharesBalance.isZero()}
                    onClick={handleWithdrawAll}
            ...
            {/* <div className={classes.zapNote}>
              ...
              <ol>
                ...
                <h4 class="mt50">ROI <span>12.12% (12days)</span></h4>

                <div className={classes.mt60}>
                  <p className={classes.pullLeft}><a href="#." class="fcGray6 " ><span class="underline">C</span>reate LP</a></p>
                  <p class="dp-inblock"><a href="#." class="fcGray6"><span class="underline">F</span>arm Contract</a></p>
                  <p className={classes.pullRight}><a href="#." class="fcGray6"><span class="underline">V</span>ault Contract</a></p>
                </div>
              </div>
          </div>
